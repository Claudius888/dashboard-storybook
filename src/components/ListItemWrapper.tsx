...
export function   ListItemWrapper({
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-2 w-[17.5rem] h-auto pt-[var(--spacing-24-duplicate)] pl-[var(--spacing-24-duplicate)] theme-bg'
      )}
    >
      <p className={cn('font-semibold text-base theme-text')}>{SectionTitle}</p>
      {data.map((item) => (
